...
from torch_geometric.datasets import Reddit, Planetoid, Amazon, Flickr
...
def main():
    ...
    if victim == 'gat':
        victim_model = GAT(in_channels=dataset.num_features, hidden_channels=victim_hidden_size, out_channels=dataset.num_classes, num_layers=2, act='relu', dropout=0.5).to(device)
    elif victim == 'gcn':
        victim_model = GCN(in_channels=dataset.num_features, hidden_channels=victim_hidden_size, out_channels=dataset.num_classes, num_layers=2, act='relu', dropout=0.5).to(device)
    ...
    encoder = Encoder(dim_in=dataset.num_features, dim_out=embedding_hidden_size, num_layers=2, conv=surrogate_conv, act=surrogate_act)
    encoder.to(device)
    
    decoder = DecoderMLP(embedding_hidden_size, dim_out=dataset.num_features, num_layers=1)
    decoder.to(device)
    ...
    for _ in tqdm(range(ssl_epochs), desc='Treino SSL', disable=tqdm_off):
        for batch in loader:
            ...
            noisy_node_feature, mask = node_mask(batch.x, batch.train_mask, std=0.5, ratio=0.05, device=device) 
    ...
    for _ in tqdm(range(surrogate_epochs), desc='Treino Classificador', disable=tqdm_off):
        for batch in loader:
